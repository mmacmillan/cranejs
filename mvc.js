...
module.exports = function(app, opt) {
    ...
    _app = app;
    _errorHandler = _opt.errorHandler;

    //** return the public interface
    return (mvc = {
        initialize: function(crane) { crane = crane; },

        //** registration methods
        controllers: function(list) {
            list = list||[]; //** normalize the args
            !Array.isArray(list) && (list = [list]);

            list.forEach(parseController);
            return mvc;
        },
...
function parseController(c) {
    if(_controllers[c.name]) return;

    //** cache the controller
    _controllers[c.name] = c; 
...
function route(handler, req, res, next) { //** simple route handler
    function error(err) { _errorHandler.call(this, err, req ,res) }
    var p = q.defer();
